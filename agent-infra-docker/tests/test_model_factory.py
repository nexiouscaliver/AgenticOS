...
class TestModelFactory:
    ...
    def test_optimal_model_selection_for_tasks(self):
        ...
        # Test research tasks
        research_model = ModelFactory.get_optimal_model(task_type=TaskType.RESEARCH, priority="budget")
        assert research_model in ["deepseek-chat", "gpt-4o-mini"], "Should select cost-effective model for research"

        # Test creative tasks
        creative_model = ModelFactory.get_optimal_model(task_type=TaskType.CREATIVE, priority="balanced")
        assert creative_model in ["gpt-4o-mini", "glm-4.5-air", "gpt-4o"], (
            "Should select appropriate model for creative tasks"
        )

        # Test coding tasks
        coding_model = ModelFactory.get_optimal_model(task_type=TaskType.CODING, priority="budget")
...
class TestCostOptimization:
    ...
    def test_budget_priority_selection(self):
        ...
        budget_model = ModelFactory.get_optimal_model(task_type=TaskType.SIMPLE, priority="budget")

        balanced_model = ModelFactory.get_optimal_model(task_type=TaskType.SIMPLE, priority="balanced")

        premium_model = ModelFactory.get_optimal_model(task_type=TaskType.SIMPLE, priority="premium")
