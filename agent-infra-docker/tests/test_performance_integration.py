...
class TestPerformance:
    ...
    def test_concurrent_agent_requests(self, api_client: requests.Session):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(make_request, agent_id, message) 
                      for agent_id, message in test_requests]
        ...
        assert successful_requests >= 3, f"At least 3 of 4 concurrent requests should succeed, got {successful_requests}"
        ...
        for result in results:
            if result["success"]:
                assert result["response_length"] > 0, f"Successful response should have content for {result['agent_id']}"
    ...
    def test_workflow_performance(self, api_client: requests.Session):
        ...
        response = api_client.post(
            ...
            json={
                "message": "Write a brief blog about artificial intelligence benefits",
                "stream": False
            },
...
class TestSystemReliability:
    ...
    def test_invalid_agent_handling(self, api_client: requests.Session):
        ...
        response = api_client.post(
            get_api_url("/agents/non-existent-agent/chat"),
    ...
    def test_invalid_workflow_handling(self, api_client: requests.Session):
        ...
        response = api_client.post(
            get_api_url("/workflows/non-existent-workflow/run"),
            json={"message": "Test message", "stream": False}
        )
    ...
    def test_empty_message_handling(self, api_client: requests.Session):
        ...
        for test_case in test_cases:
            response = api_client.post(
                ...
                json=dict(test_case, stream=False),
                timeout=30
            )
...
class TestCostOptimization:
    ...
    def test_model_selection_efficiency(self, api_client: requests.Session):
        ...
        for agent_id, max_time in agents_with_expected_speed:
            ...
            assert response.status_code == 200, f"Agent {agent_id} should respond successfully with cost-optimized model"
            assert response_time < max_time, f"Agent {agent_id} should respond quickly with cost-optimized model, took {response_time:.2f}s"
    ...
    def test_team_cost_efficiency(self, api_client: requests.Session):
        ...
        response = api_client.post(
            ...
            json={
                "message": "Quick research on renewable energy benefits", 
                "stream": False
            },
...
class TestQualityAssurance:
    ...
    def test_system_consistency(self, api_client: requests.Session):
        ...
        assert not all(r == responses[0] for r in responses[1:]), \
            "Responses should show variation, not be identical"
...
class TestDocumentationAndExamples:
    ...
    def test_workflow_usage_example(self, api_client: requests.Session):
        ...
        response = api_client.post(
            get_api_url("/workflows/simple-blog-workflow/run"),
            json={
                "message": "Create a sample blog about machine learning basics",
                "stream": False
            },
            timeout=180
        )
