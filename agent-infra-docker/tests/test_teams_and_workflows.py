...
class TestResearchTeam:
    ...
    def test_research_team_coordination(self, api_client: requests.Session):
        ...
        response = api_client.post(
            ...
            json={"message": "Research the environmental impact of renewable energy technologies", "stream": False},
    ...
    def test_research_team_quality_output(self, api_client: requests.Session):
        ...
        response = api_client.post(
            get_api_url("/teams/comprehensive-research-team/chat"),
            json={"message": "Analyze the current state of artificial intelligence in healthcare", "stream": False},
            timeout=120,
        )
...
class TestBlogWritingWorkflow:
    ...
    def test_simple_blog_workflow_execution(self, api_client: requests.Session):
        ...
        response = api_client.post(
            ...
            json={"message": "Write a blog about the benefits of remote work", "stream": False},
        ...
        content_lower = content.lower()
        assert any(indicator in content_lower for indicator in topic_indicators), "Should address the requested topic"

        # Should have blog structure elements
        blog_elements = ["introduction", "conclusion", "benefits", "advantages"]
        assert any(element in content_lower for element in blog_elements), "Should have blog structure elements"
...
class TestWorkflowIntegration:
    ...
    def test_workflow_streaming_capability(self, api_client: requests.Session):
        ...
        response = api_client.post(
            get_api_url("/workflows/simple-blog-workflow/run"),
            json={"message": "Write about artificial intelligence trends", "stream": True},
            timeout=120,
...
class TestSystemIntegration:
    ...
    def test_multi_model_support(self, api_client: requests.Session):
        ...
        agents_to_test = ["advanced-web-research-agent", "agno-documentation-expert", "content-writer-agent"]

        for agent_id in agents_to_test:
            response = api_client.post(
                get_api_url(f"/agents/{agent_id}/chat"), json={"message": "Test multi-model support", "stream": False}
            )
